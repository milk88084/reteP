...
const FEATURES: Feature[] = [
  { emoji: '🍽️', title: '記錄每一餐', desc: '食物名稱、份量、熱量與蛋白質、碳水、脂肪，依早午晚餐分類' },
  { emoji: '🍱', title: '食物庫', desc: '內建上百筆常見食物，常吃的品項可存進「我的食物」重複使用' },
  { emoji: '🔁', title: '重複偵測', desc: '輸入吃過的食物，自動帶出上次的數值，不用每次重打' },
  { emoji: '🎯', title: '今日營養推薦', desc: '分析當天攝取，點出最缺的營養素並建議可以補的食物' },
  { emoji: '🧮', title: '目標自動計算', desc: '輸入身高體重與增減重目標，依 BMR／TDEE 算出每日熱量與三大營養素目標' },
  { emoji: '📅', title: '月曆回顧與年度趨勢', desc: '用月曆逐日檢視每餐，年度圖表看每月熱量與營養的變化' },
]
    ...
    <div className="relative mx-auto max-w-6xl px-5 pt-14 pb-16 text-center sm:px-8 sm:pt-20">
      <motion.div initial="hidden" animate="show" variants={stagger}>
        ...
        <motion.div variants={fadeUp} className="mt-8 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-ink-muted">iPhone 免費下載・Apple、Google 登入</p>
        </motion.div>
